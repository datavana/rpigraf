...
``` r
api_fetch_entity(ids, params = c(), db = NULL, silent = FALSE)
```
...
- ids:
  ...
  A character vector with IDs as returned by api_fetch_table, e.g.
  articles-1. Alternatively, provide a dataframe containg the IDs in the
  id-column. So you can chain fetch_articles() and api_fetch_entity()
...
- db:
  ...
  api_fetch_table(). In this case, the database name will be extracted
  from the dataframe.
